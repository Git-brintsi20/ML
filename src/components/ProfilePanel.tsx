...
export default function ProfilePanel() {
  ...
  const handleSavePlatform = async (platform: string) => {
    await updatePlatforms({
      [platform]: platformFormData[platform as keyof typeof platformFormData],
    });
  ...
  const completedLevels = LEVELS.filter(l =>
    l.tasks.every(t => tasksChecked[t.id])
  ).length;
          ...
          >
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-3 py-2 bg-dark-bg2 border border-dark-border rounded text-dark-text focus:outline-none focus:border-accent-primary resize-none"
              />
            </div>

            <button
              type="submit"
              className="px-6 py-2 bg-accent-primary text-white rounded hover:opacity-90 transition-opacity"
            >
              Save Profile
            </button>
          </form>
      ...
      <div className="bg-dark-surface border border-dark-border rounded-lg p-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          {[
            ...
            <div key={key} className="p-4 border border-dark-border rounded">
              {editingPlatform === key ? (
                <div className="space-y-2">
                  <input
                    type="text"
                    value={platformFormData[key as keyof typeof platformFormData]}
                    onChange={(e) => setPlatformFormData({ ...platformFormData, [key]: e.target.value })}
                    placeholder="URL"
                    className="w-full px-3 py-2 bg-dark-bg2 border border-dark-border rounded text-dark-text focus:outline-none text-sm"
                  />
                  <div className="flex gap-2">
                    <button
                      onClick={() => handleSavePlatform(key)}
                      className="flex-1 px-2 py-1 bg-accent-primary text-white rounded text-sm hover:opacity-90"
                    >
                      Save
                    </button>
                    <button
                      onClick={() => setEditingPlatform(null)}
                      className="flex-1 px-2 py-1 bg-dark-bg3 border border-dark-border rounded text-sm hover:bg-dark-surface"
                    >
                      Cancel
                    </button>
                  </div>
                ...
                >
                  ...
                  {platformFormData[key as keyof typeof platformFormData] ? (
                    <div className="text-xs text-green-400 truncate">
                      ✓ {platformFormData[key as keyof typeof platformFormData].replace('https://', '').substring(0, 20)}
                    </div>
                  ) : (
                    <div className="text-xs text-accent-secondary">Add URL →</div>
                  )}
                </button>
              )}
            </div>
