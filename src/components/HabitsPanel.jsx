...
import { HABITS } from '@/app/lib/data';
function getWeekKey() {
    const d = new Date();
    const day = d.getDay();
    ...
    return (<div className="space-y-6">
      ...
      <div className="space-y-4">
        {HABITS.map(habit => {
            ...
            return (<div key={habit.id} className="bg-dark-surface border border-dark-border rounded-lg p-4">
              ...
              <div className="grid grid-cols-7 gap-2">
                {days.map((day, idx) => {
                    ...
                    return (<button key={day} onClick={() => toggleHabit(habit.id, day, weekKey)} className="flex flex-col items-center gap-1 p-2 rounded border border-dark-border hover:border-accent-primary transition-colors" title={dayLabels[idx]}>
                      <div className={`w-10 h-10 rounded flex items-center justify-center font-bold transition-all ${isHit
