...
export default function LevelsPanel() {
  ...
  const [expandedLevels, setExpandedLevels] = useState<boolean[]>(new Array(LEVELS.length).fill(true));
    ...
    <div className="space-y-6">
      ...
      <div className="bg-dark-surface border border-dark-border rounded-lg p-6">
        ...
        <div className="h-2 bg-dark-bg3 rounded-full overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-accent-primary to-teal-500 transition-all duration-500"
      ...
      <div className="space-y-4">
        {LEVELS.map((level, levelIdx) => (
          <div key={levelIdx} className="border border-dark-border rounded-lg overflow-hidden">
            {/* Level Header */}
            <button
              onClick={() => toggleLevelExpand(levelIdx)}
              className="w-full px-6 py-4 bg-dark-surface hover:bg-dark-surface2 transition-colors flex items-center gap-4"
            >
            ...
            <div className="px-6 py-2 bg-dark-bg3 border-b border-dark-border">
              <div className="h-1 bg-dark-border rounded-full overflow-hidden">
                <div
                  className="h-full bg-gradient-to-r from-accent-primary to-teal-500 transition-all"
                  style={{ width: `${levelStats[levelIdx].pct}%` }}
                />
              </div>
            </div>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-dark-border">
                    {level.tasks.map(task => (
                      <tr
                        key={task.id}
                        className={`divide-x divide-dark-border hover:bg-dark-surface2 transition-colors ${
                          tasksChecked[task.id] ? 'opacity-60' : ''
                        }`}
                      >
